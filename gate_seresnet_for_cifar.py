...
class BottleNeck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride, downsample=None):
        ...
        self.se = SEBlock(planes * self.expansion, reduction=16)
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block_type, n_block:int, n_channel:int, stride:int):
        ...
        layers = []
        layers.append(block_type(self.in_channel, n_channel, stride, downsample))

        self.in_channel = n_channel * block_type.expansion
        for _ in range(1, n_block):
            layers.append(block_type(self.in_channel, n_channel, stride=1, downsample=None))
...
def gate_seresnet34(num_classes=100):
    return ResNet(
                num_blocks = [3, 4, 6, 3],
