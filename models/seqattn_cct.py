...
class SeqAttnHead(nn.Module):
    ...
    def forward(self, x): # x: (B, N=HW, D=curr_dim)
        ...
        for k in range(self.branches):
            ...
            # SeqPool
            out = nn.Softmax(dim=-1)(self.fc_in[k](x).transpose(-2, -1)) # (B, N, curr_dim) -> (B, N, 1) -> (B, 1, N)
            out = out @ x

            # Class Attention
            out = self.gram_attention[k](x, out) # x: (B, N, curr_dim), gram: (B, 1, curr_dim) -> out: (B, 1, curr_dim)
            out = out.view(out.size()[0], -1) # (B, 1, curr_dim) -> (B, curr_dim)
...
class TransformerClassifier(Module):
    def __init__(self,
        ...
        if not seqattn:
            ...
            self.seq_attn_head = SeqAttnHead(
    ...
    def forward(self, x):
        ...
        if self.seqattn:
            x = self.seq_attn_head(x) # (B, num_classes)
        else:
            x = x[:, 0]
...
class SeqAttn_CCT(nn.Module):
    def __init__(self,
        ...
        super(SeqAttn_CCT, self).__init__()
...
def seqattn_cct_7(arch, pretrained, progress, *args, **kwargs):
    return _seqattn_cct(arch, pretrained, progress, num_layers=7, num_heads=4, mlp_ratio=2, embedding_dim=256, seqattn=True, *args, **kwargs)


@register_model
def seqattn1_cct_7_3x1_32(pretrained=False, progress=False, img_size=32, positional_embedding='learnable', num_classes=10, *args, **kwargs):
    return seqattn_cct_7(
        'seqattn1_cct_7_3x1_32',
        pretrained,
        progress,
        kernel_size=3,
        n_conv_layers=1,
...
if __name__ == '__main__':
    ...
    model = create_model('seqattn1_cct_7_3x1_32', num_classes=100)
    y = model(x)